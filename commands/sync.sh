...
PARSED_ARGUMENTS=$(getopt -a -n "escue sync-file" -o aejo:i:f:n:t: --long file:,node:,target: -- "$@")
VALID_ARGUMENTS=$?
...
case $COMMAND in
  ...
  ana        ) sync_ana $CLUSTER $FILE "$TARGET" "$INDENTY_FILE" "$SSHOPTIONS" ;;
  plugins    ) sync_plugin $CLUSTER $FILE "$TARGET" "$INDENTY_FILE" "$SSHOPTIONS" ;;
  -h | --help) usage ;;
  *) usage ;;
esac
